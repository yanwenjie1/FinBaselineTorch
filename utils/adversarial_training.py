    ...
    def attack(self, is_first_attack=False):
        for name, param in self.model.named_parameters():
            if param.requires_grad and self.emb_name in name:
                ...
                if norm != 0:
                    r_at = self.alpha * param.grad / norm  # 把0.3分配到grad的每一个参数
                    # 函数加了下划线的属于内建函数，将要改变原来的值，没有加下划线的并不会改变原来的数据，引用时需要另外赋值给其他变量
                    param.data.add_(r_at)  # 这里换句话说对embedding加了总和0.3的扰动
                    param.data = self.project(name, param.data, self.epsilon)  # 重新规范扰动的情况 防止一次次加下去偏离太多
